...
export default async (hubConfig = {}) => {
    ...
    const messageQueue = await MessageQueue({
        host: process.env.MQ_URL || hubConfig.mq_url || 'amqp://localhost'
    })
        ...
        if(jobs && jobs.length > 0){
            ...
            switch(job.type){
                case 'file-conversion':
                    ...
                    await flowProvider.put("Files", job.fileId, {$push: {
                        conversions: {
                            cid: processed_item.output_cid,
    ...
    const fileStore = await FileStore(null, pipelineManager)
