...
export default (db) => {
  ...
  return {
    ...
    removeProvider: (collection, typeDef, provides) => {
      return async (id) => {
        let query = {}
        for(var k in typeDef){
          if(typeDef[k] === "ID"){
        ...
        query = mapQuery(provides, query)
        let r = await db.collection(collection).deleteOne(query)
    ...
    provider: (collection, typeDef, provides) => {
      return async (_query) => {
        ...
        for(var k in query){
          if(typeDef[k] === "ID"){
            console.log(`ID ${k} typeof ${JSON.stringify(query)}`)
